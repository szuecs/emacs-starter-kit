...
;; 
;; # as in RDoc documentation block.yasnippet
;; $TM_LINE_INDEX                                                                             =yyas> (yas/unknown)
;; 
;; # as in RDoc documentation block.yasnippet
;; `[[ $TM_LINE_INDEX != 0 ]] && echo; echo`                                                  =yyas> (yas/unknown)
;; 
;; # as in Wrap in Begin Rescue End.yasnippet
;; ${TM_SELECTED_TEXT/(\A.*)|(.+)|\n\z/(?1:$0:(?2:\t$0))/g}                                   =yyas> (yas/unknown)
;; 
;; # as in class .. instance_methods .. undef .. initialize .. end (class).yasnippet
;; ${2/(^.*?\S.*)|.*/(?1:\()/}                                                                =yyas> (yas/unknown)
;; 
;; # as in flunk(..) (fl).yasnippet
;; `snippet_paren.rb`                                                                         =yyas> (yas/unknown)
;; 
;; # as in Wrap in Begin Rescue End.yasnippet
;; ${2/.+/ => /}                                                                              =yyas> (yas/unknown)
;; 
;; # as in open yield block ({).yasnippet
;; ${1/(^(?<var>\s*[a-z_][a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:|)/}                      =yyas> (yas/unknown)
;; 
;; # as in do obj .. end (doo).yasnippet
;; ${1/(^(?<var>\s*[a-z_][a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1: |)/}                     =yyas> (yas/unknown)
;; 
;; # as in class .. instance_methods .. undef .. initialize .. end (class).yasnippet
;; ${2/(^.*?\S.*)|.*/(?1:\))/}                                                                =yyas> (yas/unknown)
;; 
;; # as in module .. module_function .. end.yasnippet
;; ${TM_FILENAME/(?:\A|_)([A-Za-z0-9]+)(?:\.rb)?/(?2::\u$1)/g}                                =yyas> (yas/unknown)
;; 
;; # as in upto(1.00.0) { n .. } (upt).yasnippet
;; ${2/(^(?<var>\s*(?:\*|\*?[a-z_])[a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:|)/}            =yyas> (yas/unknown)
;; 
;; # as in #usrlocalbinruby -w.yasnippet
;; ${TM_RUBY_SWITCHES: -wKU}                                                                  =yyas> (yas/unknown)
;; 
;; # as in transaction( __ ) do __ end.yasnippet
;; ${1/(^.*?\S.*)|.*/(?1:\()/}                                                                =yyas> (yas/unknown)
;; 
;; # as in class .. ParentClass .. initialize .. end.yasnippet
;; ${3/(^.*?\S.*)|.*/(?1:\))/}                                                                =yyas> (yas/unknown)
;; 
;; # as in transaction( __ ) do __ end.yasnippet
;; ${1/(^.*?\S.*)|.*/(?1:\))/}                                                                =yyas> (yas/unknown)
;; 
;; # as in namespace __ do __ end.yasnippet
;; ${TM_FILENAME/\.\w+//}                                                                     =yyas> (yas/unknown)
;; 
;; # as in class .. TestUnitTestCase .. end (tc).yasnippet
;; ${1/([\w&&[^_]]+)|./\u$1/g}                                                                =yyas> (yas/unknown)
;; 
;; # as in open(pathorurl, w) do doc .. end (ope).yasnippet
;; ${3/(^[rwab+]+$)|.*/(?1:, ")/}                                                             =yyas> (yas/unknown)
;; 
;; # as in flunk(..) (fl).yasnippet
;; `snippet_paren.rb end`                                                                     =yyas> (yas/unknown)
;; 
;; # as in open yield block ({).yasnippet
;; ${1/(^(?<var>\s*[a-z_][a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:| )/}                     =yyas> (yas/unknown)
;; 
;; # as in inject(init) { mem, var .. } (inj).yasnippet
;; ${1/.+/(/}                                                                                 =yyas> (yas/unknown)
;; 
;; # as in upto(1.00.0) { n .. } (upt).yasnippet
;; ${2/(^(?<var>\s*(?:\*|\*?[a-z_])[a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:| )/}           =yyas> (yas/unknown)
...
;; ${3/(^[rwab+]+$)|.*/(?1:")/}                                                               =yyas> (yas/unknown)
;; 
;; # as in option_parse { .. } (optp).yasnippet
;; ${2/^\s*$|(.*\S.*)/(?1: )/}                                                                =yyas> (yas/unknown)
;; 
;; # as in times { n .. } (tim).yasnippet
;; ${1/(^(?<var>\s*(?:\*|\*?[a-z_])[a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:| )/}           =yyas> (yas/unknown)
;; 
;; # as in inject(init) { mem, var .. } (inj).yasnippet
;; ${1/.+/)/}                                                                                 =yyas> (yas/unknown)
...
;; 
;; # as in zip(enums) { row .. } (zip).yasnippet
;;                                                                                            =yyas> (yas/unknown)
;; 
;; # as in Insert ERb's __ or = __.yasnippet
;; text.html, source.yaml, meta.erb                                                           =yyas> (yas/unknown)
;; 
;; # as in embed string variable.yasnippet
;; (string.quoted.double.ruby|string.interpolated.ruby) - string source                       =yyas> (yas/unknown)
;; 
;; 
