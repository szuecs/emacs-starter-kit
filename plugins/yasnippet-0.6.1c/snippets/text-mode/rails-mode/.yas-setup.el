...
;; 
;; # as in image_submit_tag.yasnippet
;; ${1/^(\w+)(\.\w*)?$/$1/}                                                                   =yyas> (yas/unknown)
;; 
;; # as in returning do 7Cvariable7C E280A6 end.yasnippet
;; ${2/(^(?<var>\s*[a-z_][a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1:|)/}                      =yyas> (yas/unknown)
;; 
;; # as in Create sweeper class.yasnippet
;; ${1/./\l$0/}                                                                               =yyas> (yas/unknown)
;; 
;; # as in has_one (ho).yasnippet
;; ${1/[[:alpha:]]+|(_)/(?1::\u$0)/g}                                                         =yyas> (yas/unknown)
;; 
;; # as in find_in_batches.yasnippet
;; ${TM_CURRENT_WORD/(\w+)\./\L$1/g}                                                          =yyas> (yas/unknown)
;; 
;; # as in returning do 7Cvariable7C E280A6 end.yasnippet
;; ${2/(^(?<var>\s*[a-z_][a-zA-Z0-9_]*\s*)(,\g<var>)*,?\s*$)|.*/(?1: |)/}                     =yyas> (yas/unknown)
;; 
;; # as in form_for label.yasnippet
;; ${1/[[:alpha:]]+|(_)/(?1: :\u$0)/g}                                                        =yyas> (yas/unknown)
;; 
;; 
...
;; 
;; # as in respond_with.yasnippet
;; meta.rails.controller                                                                      =yyas> (yas/unknown)
;; 
;; # as in verify - redirect (verify).yasnippet
;;                                                                                            =yyas> (yas/unknown)
;; 
;; # as in resources.yasnippet
;; meta.rails.routes                                                                          =yyas> (yas/unknown)
;; 
;; # as in licai.yasnippet
;; text.html.ruby, text.haml                                                                  =yyas> (yas/unknown)
;; 
;; # as in def post request.yasnippet
;; meta.rails.functional_test                                                                 =yyas> (yas/unknown)
;; 
;; # as in Migration Create Column Continue (mccc).yasnippet
;; meta.rails.migration.create_table                                                          =yyas> (yas/unknown)
;; 
;; # as in Table column(s) rename.yasnippet
;; meta.rails.migration.change_table                                                          =yyas> (yas/unknown)
;; 
;; # as in t_timestamps (tctss).yasnippet
;; meta.rails.migration.create_table, meta.rails.migration.change_table                       =yyas> (yas/unknown)
;; 
;; # as in Migration Remove and Add Column (mrac).yasnippet
;; meta.rails.migration - meta.rails.migration.create_table - meta.rails.migration.change_table  =yyas> (yas/unknown)
;; 
;; # as in if (ERB).yasnippet
...
;; 
;; # as in rails session.yasnippet
;; ^j                                                                                         =yyas> (yas/unknown)
;; 
;; # as in rails params.yasnippet
;; ^p                                                                                         =yyas> (yas/unknown)
;; 
;; 

;; .yas-setup.el for Ruby ends here
